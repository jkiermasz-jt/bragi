...
struct IdentifierSanitizer {
  ...
  func sanitize(_ identifier: String) -> String {
    ...
    let lowercasedKeywords: Set<String> = Set(Self.swiftKeywords.map { $0.lowercased() })

    if lowercasedKeywords.contains(result.lowercased()) {
      let canonicalKeyword = findCanonicalKeyword(for: result)

      let innerResult: String = if capitalized {
        canonicalKeyword.prefix(1).uppercased() + canonicalKeyword.dropFirst()
      } else {
        canonicalKeyword.prefix(1).lowercased() + canonicalKeyword.dropFirst()
      }
